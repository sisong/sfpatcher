...
压缩算法破坏了修改“现场”，diff算法的优势无法真正发挥出来。那么我们是否可以开发针对apk文件格式的diff&patch算法呢？ 它能够识别出上面的情况：其实只是修改了一个字符！  
...
- patch时的内存等资源占用可控；支持多种方案限制patch时的内存占用到合适的约定水平。
...
# sfpatcher的大规模测试
从一个apk网站的Top下载数据中下载了多个app应用和其多个历史版本，形成了4280个测试用例，进行了diff并使用了lzma2压缩； 后来重新下载了更多数据形成了6495个测试用例，并使用了zstd压缩输出补丁。   
